...
> **NOTE:** The LuLo files referred to here are those contained inside
...
can be installed there and coexist with a BIOS installation of LuLo
...
## Secure Boot
LuLo can be booted with secure boot if the executable is signed and the key
used to sign it is added to the firmware's keychain. This should be done in
combination with enrolling the BLAKE3 hash of the LuLo config file into the
LuLo EFI executable image itself for verification purposes.
For more information see the `limine enroll-config` program and
[the FAQ](FAQ.md).

## BIOS/MBR
In order to install LuLo on a MBR device (which can just be a raw image
...
In order to create a hybrid ISO with LuLo, place the
...
relevant LuLo EFI executables over (such as `BOOTX64.EFI`).
...
In order to boot LuLo from PXE it is necessary to setup a DHCP server with
...
The steps needed to boot LuLo are the same as with BIOS PXE,
except that the `limine-bios.sys` file is not needed on the server.

## Configuration
The `limine.conf` file contains LuLo's configuration.
