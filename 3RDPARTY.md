...
The LuLo bootloader is a fork of Limine and depends on several other projects.
...
alongside the core LuLo code in the tarballs themselves, similar to
...
that LuLo is distributed under:

A non-binding, informal summary of all projects LuLo depends on, and the
...
upstream bootloader that LuLo is forked from.
...
the one chosen by the LuLo developers. Refer to each individual project's
